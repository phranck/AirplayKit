...
#
#  AirplayKit.h is a published interface, and podlive-macos is its one known
...
caller="${AIRPLAYKIT_CALLER:-$root/../Podlive/podlive-macos}"

if [[ ! -d "$caller" ]]; then
    echo "There is no caller at $caller, so no application was built against this header."
    echo "Point AIRPLAYKIT_CALLER at a podlive-macos checkout to have it checked."
    exit 0
fi
...
reference="$(sed -n 's/^[[:space:]]*relativePath = \(.*AirplayKit\);$/\1/p' \
    "$project/project.pbxproj" | head -1)"

if [[ -z "$reference" ]]; then
    if ! grep -q 'repositoryURL = "https://github.com/phranck/AirplayKit.git";' \
        "$project/project.pbxproj"; then
        echo "$project names neither this checkout nor the AirplayKit GitHub package." >&2
        exit 1
    fi
    echo "== $caller, which compiles the released AirplayKit package from GitHub"
