...
## 2026-09-24, two receivers in one iPhone AirPlay group

**What was done.** Ran `openairplay/airplay2-receiver` as `PlayableProbe` on the local Mac and `ProbeTwo` on the MacServer, each with its own advertised hardware address. An iPhone on iOS 18.7.10 played to the first receiver, added the second, then removed it. Both receivers printed their decrypted RTSP control requests. The first receiver was also grouped with a Sonos Bookshelf in a separate run. The operator confirmed that the two test receivers were separately selectable. Their audio output was silent or distorted, so this run measures control and timing messages, not audible synchronisation.

**F-119 Two members receive different anchor values that describe the same media timeline (confirmed).** The iPhone opened a separate RTSP session to each receiver and sent the following `SETRATEANCHORTIME` bodies on their 48 kHz buffered streams:

| Field | PlayableProbe | ProbeTwo |
|---|---:|---:|
| `networkTimeSecs` | 65336 | 65343 |
| `networkTimeFrac` | 524805257272754176 | -4325102014238294016 |
| `networkTimeTimelineID` | -2267141381506007032 | -2267141381506007032 |
| `rtpTime` | 426059306 | 426430686 |

The time fields differ by 7.737086 seconds after interpreting the fraction as an unsigned 64 bit binary fraction. The RTP positions differ by 371380 frames, or 7.737083 seconds at 48000 frames per second. Their mapping differs by about 2.7 microseconds. The two bodies are therefore not identical, but they place the same media on the same clock. This overturns the untested identical-anchor premise in issue #18; the requirement for a group is an equivalent mapping, not identical field values.

**F-120 A member leaving is paused, flushed and torn down on its own session (confirmed).** After `ProbeTwo` was deselected, its control channel received `SETRATEANCHORTIME` with only `{'rate': 0}`, then `FLUSHBUFFERED` with `flushUntilSeq` and `flushUntilTS`, then `TEARDOWN` naming its buffered stream. The remaining member's session is separate. In the earlier Sonos run, the remaining test receiver received a shortened `SETPEERS` list after the Sonos left. The two receiver logs do not establish that the shortened list reached the remaining member in this particular two-probe run, because its control output was not preserved without gaps.

**F-121 A group change can rebuild the audio stream and set a fresh anchor (confirmed).** When the iPhone grouped `PlayableProbe` with the Sonos Bookshelf, the test receiver first got a longer `SETPEERS` list. Its 48 kHz buffered stream was then torn down, a 44.1 kHz buffered stream was opened, and a new full `SETRATEANCHORTIME` named another clock identity. F-034 observed a join without those extra commands, so it describes that one join, not every join. A change in negotiated format and clock accompanied this rebuild; this run does not isolate which change caused it.

**F-122 (method) This Python receiver cannot validate audible group synchronisation (confirmed).** Its README says accurate PTP synchronisation is not implemented. During the Sonos group run the listener heard crackle and the receiver printed repeated `InvalidDataError` from `avcodec_send_packet()`. During the two-probe run the listener heard no audio. The decrypted control data remains usable, but the physical sync check must use real receivers.

**F-123 (method) Two copies of this receiver publish the same `pi`, so their advertised group identity is not evidence of playback grouping (confirmed).** Discovery labelled `PlayableProbe` and `ProbeTwo` as sharing a group before either was selected. Both copies had the same built-in public identifier. This is a test fixture collision, not evidence that the iPhone had grouped them, and it reinforces the limit on interpreting `gid` from F-087 and F-093.

**F-124 Two Sonos receivers did not converge on one clock from a shared group identifier and `SETPEERS` alone (confirmed for this probe).** A temporary diagnostic in `Demo` paired separately with Büro (Bookshelf, `10.0.0.125`) and Esszimmer (One, `10.0.0.112`). It gave both session SETUP bodies the same `groupUUID`, advertised sender `ClockID` and timing peer `ID`, opened each event and buffered stream channel, then sent each receiver `SETPEERS` with the sender's `10.0.0.193` and the other receiver's IPv4 address. On two successive reads, Büro announced grandmaster `542a1bfffe58d1f8` at about 164647 seconds, while Esszimmer announced `48a6b8fffef7ca56` at about 1479229 seconds. After a further wait Esszimmer still announced its own identity and Büro yielded no complete reading. The probe sent no audio or anchor, and its peer lists lacked the other devices' IPv6 addresses. It therefore establishes that this particular minimal exchange did not produce a shared clock, not which missing action would make it do so. The diagnostic was removed after the run.

**F-125 An active sender PTP clock supplies the missing shared time to two Sonos receivers (confirmed for this probe).** A new original Swift PTP grandmaster bound UDP 319 and 320, sent Sync, Follow_Up and Announce to both receivers and answered their Delay_Req messages. With the shared sender clock in both session SETUP bodies, both receivers sent Delay_Req to it: 27 each in the first no-audio probe, and both returned a usable anchor. This differs from F-124, where the sender advertised a clock but sent no PTP traffic. The probe did not establish that every AirPlay 2 receiver accepts this clock profile.

**F-126 Two and three real receivers played one grouped source (confirmed by operator listening).** `swift run Demo group-tone` sent a louder 440 Hz test tone for ten seconds to Büro and Esszimmer. Both receivers returned an anchor and each sent 119 Delay_Req messages. The operator reported both audible and simultaneous. `swift run Demo group-tone3` then sent twenty seconds to Büro, Esszimmer and Badezimmer (Sonos One, `10.0.0.196`). Each sent 195 Delay_Req messages and returned an anchor; the operator replied "perfekt" to the three-speaker test. This is an acoustic judgement, not a measured inter-speaker delay, and the packet counts alone cannot prove acoustic synchronisation.

**F-127 Dynamic membership works in repeated local runs, but a prior failure is not fully explained (confirmed for these runs).** `swift run Demo group-api Büro Esszimmer Badezimmer 16` opened a two-member group, added Badezimmer after five seconds and removed Esszimmer after another five while sending the same tone. The first run ended after the join with a generic failure; its cause was not captured. The second run completed. After an additional full-session `TEARDOWN` was added on departure, two runs ended with `event channel 38420B60C6CE: the event connection closed` and an empty member list after removing Esszimmer. The extra `TEARDOWN` was removed to match the measured leaving sequence in F-120, and the departing member's expected channel stop no longer ends the group. Two subsequent 16-second runs completed with Büro and Badezimmer still in the group after Esszimmer left. Because both cleanup changes were made together and the earlier first run also failed, this is not proof of a single root cause or broad reliability.

**F-128 Physical volume buttons produced typed events during an open session (confirmed on one Sonos).** `swift run Demo volume-watch 10.0.0.125 30` opened a silent session with Büro while the operator pressed its volume buttons up and down. The first read was 0.54. `volumeChanged` rose through 0.56 to 0.9800001 and then fell through 0.94 to 0.62, which was also the final level. This verifies that a changed receiver volume can reach the Swift typed callback while PlayableAirplay owns a session. It does not show an idle receiver's volume without an open session.

**F-129 A group can start on one Sonos and add two more without reopening its first member (confirmed in two local runs).** `swift run Demo group-api Büro Esszimmer Badezimmer 20` opened `AirPlayGroup` with only Büro, began the tone producer, added Esszimmer after five seconds, added Badezimmer after another five and removed Esszimmer after another five. Both runs printed membership IDs `[542A1B58D1F8]`, then `[542A1B58D1F8, 48A6B8F7CA56]`, then all three including `38420B60C6CE`, and finally Büro and Badezimmer, and exited successfully. The second run included the change that distinguishes an intentional socket stop from a failed write during removal. This verifies the API and control sequence on those devices. No operator listening report was requested for these runs, and they do not prove gapless acoustic playback at the transitions.

**F-130 The HomePod mini refused this sender's first transient pairing request (confirmed for this device and setting).** With the operator's permission, `swift run Demo group-tone 10.0.0.125 Emma.local. 10` attempted a mixed Büro and HomePod mini group on 24 September. No tone started: the HomePod answered `403 Forbidden` to the first `POST /pair-setup` (`CSeq: 1`). A separate single-receiver `Demo play Emma.local. 7000 2` also failed during pairing. One diagnostic run first sent an accepted `GET /info` on the same connection and then got the same 403 on pair-setup at `CSeq: 2`. Another added `X-Apple-Client-ID` with the sender's random session identifier and still got 403 at `CSeq: 1`. Changing only the User-Agent to the `AirPlay/1005.7.1` observed from macOS, changing the pair request line from `RTSP/1.0` to `HTTP/1.1`, and connecting directly to its IPv4 address in separate runs each left the 403 unchanged. A six-byte M1 without transient flags under `X-Apple-HKP: 3` also got an immediate 403. Finally, the first request was made in the header form observed from a successful Mac pairing in `build/captures/macserver/session.log`: `X-Apple-HKP: 4`, a fresh UUID-shaped `X-Apple-Client-ID`, `Content-Type: application/x-apple-binary-plist`, `CSeq: 1`, `User-Agent: AirPlay/1005.7.1`, and the same nine-byte TLV M1. Emma still answered 403. All diagnostic code changes were removed. The operator clarified that earlier successful HomePod audio came from Eter Radio on this Mac, not PlayableAirplay. This localizes the refusal before session SETUP, PTP and grouping, but does not establish whether the cause is HomePod access settings, an authenticated sender identity or another pairing condition. No mixed-brand playback was measured.

**F-131 This HomePod advertises a newer OS release than in the first group capture (confirmed).** A fresh `_raop._tcp` browse on 24 September read `ov=27.0`, `sf=0x98404`, `et=0,3,5` and no `pw` field for Emma. The same hardware had advertised `ov=26.6` in the 22 September measurement. This identifies a changed receiver environment, not a cause for F-130: no pairing run on this library was recorded against its earlier release, and the absence of `pw` does not establish the current Home app access rule. A direct RTSP `GET /info` still answered 200 and supplied its name and model.

**F-132 The AirPlay record advertises `acl=1` for Emma, but the enforced rule is not yet measured (confirmed advertisement, open interpretation here).** A fresh `_airplay._tcp` browse read `acl=1` for the HomePod and Apple TV, `acl=0` for Büro, and `acl=1` for Esszimmer. [HAP-NodeJS AccessLevel](https://developers.homebridge.io/HAP-NodeJS/enums/AccessLevel.html) maps 1 to "Only People Sharing This Home" and 0 to either open or same-network access. That is a published implementation's mapping, not a direct reading of Emma's Home app setting. In particular, Esszimmer accepted this sender's transient pairing in F-126 despite advertising 1, so the value alone cannot prove that the HomePod's 403 comes from this policy. Reading the current Home app setting and changing only that setting for a controlled retest would settle the local cause.

**F-133 A second Apple receiver with `acl=1` also refused the same first transient request (confirmed for this probe).** A raw `POST /pair-setup` M1 containing State 1, Method 0, Flags 0x10 and `X-Apple-HKP: 4` was sent to the Apple TV at `Wohnzimmer-2.local.:7000`, without opening a playback session. It answered `RTSP/1.0 403 Forbidden`, as Emma did. This narrows the comparison from one possibly faulty HomePod to two Apple receivers advertising the same access level, but it does not by itself prove which policy caused the refusal. The Sonos counterexample in F-132 remains.

**F-134 The previously captured successful macOS group used stored-credential pair-verify on both Apple receivers (confirmed by re-reading the local packet capture).** `build/captures/airplay-group-20260922-082029.pcap` contains `POST /pair-verify RTSP/1.0` with `X-Apple-HKP: 6` from this Mac to each Apple receiver, followed by 200 answers. No `POST /pair-setup` occurred in that capture (F-022). The successful Apple sender path is therefore different from this library's anonymous transient request. It demonstrates that the HomePod is reachable and accepts a sender that already has pairing credentials; it does not show that a new third-party sender can acquire those credentials while the current access policy is active. The accepted request's `X-Apple-Client-ID` had UUID syntax, whereas the earlier F-130 diagnostic added a hexadecimal session identifier, so that diagnostic did not reproduce Apple's exact identity field. The iPhone's `X-Apple-HKP: 8` in F-021 belongs to a different sender and capture.

**F-135 A Home app access change was followed by successful transient pairing and a mixed-brand group probe (confirmed for this run; acoustic result pending).** The operator changed the home-wide speaker access setting for a controlled test. Emma's `_airplay._tcp` record changed from `acl=1` in F-132 to `acl=0`. The first unchanged `Demo group-tone` run after the announcement changed still got 403 on `POST /pair-setup`. A later raw M1 in the same header order and with the same fields as this sender got 200, as did variants of the previously captured Mac request. Without changing the library, `swift run Demo group-tone 10.0.0.125 Emma.local. 10` then exited successfully: Büro and Emma accepted the same group anchor, and sent 130 and 132 Delay_Req messages to the sender clock. The probe wrote the same 440 Hz audio packets to both for ten seconds, but process exit and PTP counts do not establish that either rendered audio or that they sounded synchronous. The operator's listening report and the restoration of the original access setting are still pending. The first 403 after `acl=0` means this run does not establish exactly when the receiver began enforcing the new setting; propagation delay is a hypothesis, not a measurement.

**F-136 Eter Radio reused a verified pairing with Emma rather than starting transient pair-setup (confirmed for this capture).** The operator captured Eter's successful connection on this Mac in `/tmp/playableairplay-eter-emma.pcap` on 24 September. The visible control requests include `GET /info?txtAirPlay&txtRAOP` and two `POST /pair-verify RTSP/1.0` requests, each with `X-Apple-HKP: 6` and a 200 response. No `POST /pair-setup` appears in the capture. This independently reproduces the stored-credential path seen in F-134 and explains why Eter's successful audio does not demonstrate that this library's fresh transient pairing should be admitted under the home-members-only rule. The capture does not reveal how Eter or macOS originally obtained its pairing credentials.

**F-137 A 30-second mixed Sonos and HomePod group probe completed under the open Home access setting (confirmed transport; acoustic result pending).** After the operator restored the original Home setting, Emma again advertised `acl=1`; a fresh `Demo play Emma.local. 7000 1` then reported that the receiver refused pairing. The operator temporarily opened access again, and Emma's advertisement changed to `acl=0`. An initial `Demo group-tone ... 30` attempt exited with code 2 before connecting because the demo command capped this probe at ten seconds. The cap was raised to 30 seconds, matching the existing three-receiver probe limit, without changing the sender. `swift run Demo group-tone 10.0.0.125 Emma.local. 30` then exited successfully: Büro and Emma accepted anchors and sent 282 and 285 Delay_Req messages respectively. The operator's acoustic report and second restoration of the Home setting are pending. Exit status and timing traffic alone do not prove that the speakers rendered audible, synchronous sound.

**F-138 The operator heard the 30-second Büro and Emma group as audible and simultaneous; the restored Home access rule rejected fresh pairing again (confirmed for this run).** After F-137, the operator answered yes to the combined question of whether both speakers were audible and sounded simultaneous. This is a listening judgement, not an instrumented acoustic offset. The operator then restored the prior Home app rule. Emma's live `_airplay._tcp` announcement returned to `acl=1`, and an unchanged `swift run Demo pair Emma.local. 7000` received `403 Forbidden` on its first `POST /pair-setup` (`CSeq: 1`). The sequence `acl=1` plus 403, then `acl=0` plus successful playback, then `acl=1` plus 403 is strong causal evidence that this Home access rule gates this sender's fresh transient pairing. It does not establish that `acl` is enforced the same way by other brands or that an authorized long-term pairing cannot work under the restrictive rule.

**F-139 (method) Podlive's group adapter completed a live join and leave sequence on two Sonos receivers (confirmed for this run).** On 24 September, a temporary macOS XCTest drove Podlive's `LWAirPlayOutput`, which uses the public `PAGroup` C API, against Büro and Esszimmer. It first opened an API group to read both existing volume levels and closed that snapshot. It then opened Podlive output with Büro, supplied 44.1 kHz stereo PCM for twenty seconds, asked Esszimmer to join after about four seconds, and asked Büro to leave after about fourteen seconds. The test checked that the requests returned without error and that one group member remained at the end; `Scripts/run-tests.sh LWAirPlayGroupSelectionTests/testLiveGroupOutputOnOfficeAndDiningRoom` reported a passing test. The test requested restoration of the saved volume levels before closing the output. Its temporary method was removed after the run, leaving the hardware-independent group selection tests in the suite. This confirms that Podlive can exercise the library's join and leave path on those receivers. The test result alone does not prove audibility, uninterrupted sound, or acoustic synchronisation at the transitions; the operator's listening report is pending.
...
4. Exact inter-speaker delay on two or three real receivers. F-126 and F-138 record audible, operator-judged simultaneous playback on Sonos groups and a mixed Sonos/HomePod mini group, but no acoustic timing measurement. Linux playback remains unvalidated. Under the restored home-members-only rule, fresh transient pairing to this HomePod is refused again; whether an authorized stored pairing can work there is still open.
5. Which exact PTP profile an Apple group sender uses. F-125 establishes that this sender's Sync, Follow_Up, Announce and Delay_Req handling provided one usable clock to two Sonos receivers, not that it duplicates Apple's every message or election rule.
6. What an Apple sender transmits as PTP. F-125 shows an active sender clock works on the tested Sonos receivers; it does not reveal Apple's own sender packets in the off-machine capture.
...
10. Which additional peer and timing information other receiver models require. F-124 shows that a shared group UUID, sender identity and IPv4 `SETPEERS` lists alone did not converge on two Sonos; F-125 and F-126 show that an active sender PTP clock made the tested Sonos group playable. An Apple group capture with sender PTP source identities and full peer lists is still needed to describe its exact role.
11. Why the first public group join run ended unexpectedly. F-127 establishes one failure and one completed rerun, but the first run lacked a captured cause. A further device run with `endedBecause` recording must settle this before reliability can be claimed.
12. How to observe a group created or changed by another controller across receiver brands. Bonjour `gid` is not sufficient on the tested Sonos speakers. No vendor-neutral AP2 event carrying complete external group topology has been established here.
7. Whether every remaining member receives a shortened `SETPEERS` after another member leaves. F-120 establishes the leaving member's pause, flush and stream teardown; the remaining channel was not preserved without gaps in that run.
8. Why the transport is IPv6 link-local in one direction and IPv4 in the other.

## 2026-09-24 to 2026-09-25, optional receiver volume memory

**What was done.** Added an application-owned JSON store keyed by the stable receiver ID, connected it to the single and group senders before playback, and exposed it through Swift and C. Ran isolated temporary-file tests on macOS, compiled the package and its tests on Linux, and built Podlive against the local C header. No receiver was connected for this check, in keeping with the operator's request to stop physical tests.

**F-140 (method) The new store round-trips confirmed levels and rejects corrupt input without replacing it (confirmed in local tests).** `swift test --filter 'ReceiverVolumeMemoryTests|SessionTests.testSwiftVolumeMemory'` passed the tests for two distinct receiver IDs, a second store instance, disabled mode, restoration, corrupt JSON and a blocked write path. The blocked write returned failure, left the in-memory saved value unchanged and exposed an error description. The Objective-C caller tests `LWAirPlayOutputConversionTests/testCVolumeMemoryDefaultsToOffAndCanBeEnabledWithoutCreatingAFile` and `LWAirPlayOutputConversionTests/testCVolumeMemoryRejectsCorruptFileWithoutReplacingIt` passed through the published C interface. These tests establish storage and interface behaviour, not that a real receiver accepted a restored volume.

**F-141 (method) A direct Swift test of the C-exported memory functions crashed the Linux Swift 6.2.4 compiler (confirmed).** The first Linux `Scripts/check-linux.sh` run compiled the sender and public library, then failed while compiling `SessionTests.swift` with a SIL type mismatch between `Optional<OpaquePointer>` from the C header and `Optional<UnsafeMutableRawPointer>` in the Swift export for `pa_volume_memory_set_enabled`. Moving the C behaviour test to the Objective-C caller and removing those direct Swift-to-C calls made the same Linux gate compile debug tests and release code successfully. This is a compiler-boundary limitation of that test arrangement; it does not establish a runtime failure of the C API.

## 2026-09-25, model labels during discovery

**What was done.** Ran `swift run Demo list` for its five-second browse without connecting to a receiver. Compared successive published snapshots and tested Podlive's model subtitle against an incomplete and a complete record with the same receiver ID.

**F-142 A receiver's short RAOP label can precede its full AirPlay product label (confirmed on this network).** The first snapshot showed Sonos receivers as `Bookshelf`, `One` and `Arc` with `isFullyDescribed=false`. Later snapshots of the same receivers showed `Sonos Bookshelf`, `Sonos One` and `Sonos Arc` with `isFullyDescribed=true`; the AirPlay service had supplied `manufacturer=Sonos`. Podlive now withholds a raw label while the record is incomplete and displays the API's product name when a complete record arrives. `Scripts/run-tests.sh LWAirPlayGroupSelectionTests/testModelSubtitleWaitsForTheCompleteAirPlayDescription` passed for this transition, and the whole `LWAirPlayGroupSelectionTests` class passed 21 tests. This run does not establish that the second service always resolves. Nor does the completed AirPlay record identify IKEA's SYMFONISK brand or a non-advertising One SL satellite.

**F-143 A plain AirPlay `/info` request does not add the missing IKEA brand on the tested Bookshelf (confirmed for this receiver).** Python `urllib.request` fetched `/info` from Büro and a Sonos Arc on port 7000 without pairing or playback; `plistlib` decoded the binary property lists. Büro returned `manufacturer=Sonos`, `model=Bookshelf`, `name=Büro`; the Arc returned `manufacturer=Sonos`, `model=Arc`. Filtering all scalar keys containing `model`, `manufact`, `brand`, `product` or `name` found no fuller model field in either response. This does not rule out other AirPlay messages or models carrying richer metadata. It does show that replacing Bonjour model labels with `/info` labels would not identify this SYMFONISK Bookshelf.

**F-144 (method) Withholding the published model made Podlive's subtitles disappear (confirmed regression, corrected).** The first implementation made `productNameForRow:` return no subtitle while `isFullyDescribed` was false. The operator's screenshot showed the visible third-party receivers without model subtitles. Discovery does not guarantee that a second Bonjour service will resolve before a row is drawn, so the initial `Bookshelf` or `One` is useful provisional data. The regression test now requires that provisional label and then `Sonos One` when the full AirPlay record arrives. It failed with the early return and passed after that return was removed. F-142 describes the earlier behavior at that point in time, not the current behavior.

**F-145 A standard UPnP description can supply a fuller model name than AirPlay on the tested Bookshelf (confirmed for this receiver; generality untested on hardware).** A read-only SSDP M-SEARCH for `upnp:rootdevice` returned a `LOCATION` from the same IPv4 address as Büro, `10.0.0.125`, pointing to its device description. The new resolver accepted the location only when its host matched the replying address, read the root device's `modelName`, and returned `SYMFONISK Bookshelf` in a live, temporary XCTest call. The test did not open an audio session or play sound. Six persistent tests cover the root-versus-nested XML choice, a second synthetic manufacturer, AirPlay `/info`, the preference order, the address check and the fallback. Podlive's new subtitle test first failed because the browser had no resolved-name store, then passed once it read the package result. The mechanism uses the standard SSDP discovery and root device field without a Sonos-specific table; whether another physical AirPlay receiver publishes a compatible UPnP description remains open. Receivers without one retain an AirPlay-derived model label. Redirects are refused when fetching metadata so a description on a receiver cannot silently move the request to another host.
