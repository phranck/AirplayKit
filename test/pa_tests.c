...
static void splitsAnInstanceIntoAddressAndName(void) {
    ...
    pa_split_instance_name("48A6B8F7CA56@Esszimmer", identifier, sizeof(identifier), name, sizeof(name));
    CHECK(strcmp(identifier, "48A6B8F7CA56") == 0, "the address is what stands before the separator");
    CHECK(strcmp(name, "Esszimmer") == 0, "the name is what stands after it");
}

static void keepsANameThatCarriesNoAddress(void) {
    char identifier[PA_MAX_ID], name[PA_MAX_NAME];

    pa_split_instance_name("Wohnzimmer", identifier, sizeof(identifier), name, sizeof(name));
    CHECK(strcmp(identifier, "Wohnzimmer") == 0, "an instance without a separator identifies itself");
    CHECK(strcmp(name, "Wohnzimmer") == 0, "and names itself");
}
...
static void keepsASeparatorInsideTheName(void) {
    ...
    pa_split_instance_name("AABBCC@Emma@Home", identifier, sizeof(identifier), name, sizeof(name));
    CHECK(strcmp(identifier, "AABBCC") == 0, "only the first separator divides");
    CHECK(strcmp(name, "Emma@Home") == 0, "the rest belongs to the name");
}
