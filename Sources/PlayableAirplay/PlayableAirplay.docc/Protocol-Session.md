...
**`streamConnectionID` is not optional on every receiver, whatever that capture shows.** A SYMFONISK Bookshelf refused a dictionary carrying every other key above with `400 Bad Request`, on both stream types, and accepted both the moment that one key was added and nothing else changed (measured 2026-09-23, from a sender written here, F-098). So a sender that omits it because an Apple capture omitted it meets a refusal with nothing in it to say which key is missing.
