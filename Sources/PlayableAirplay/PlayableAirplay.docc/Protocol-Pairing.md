...
An AirPlay receiver wants the ordinary `Control-Salt` derivation over the full 64 bytes, with nothing truncated. The transient path and the pair-verify path derive the channel keys identically, and the only thing transient pairing changes is which secret goes in. Four implementations say so and none disagrees, and each is cited at the sentence that names what it does.
