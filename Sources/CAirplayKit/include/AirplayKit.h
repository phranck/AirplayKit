...
/*
 ...
 podlive-macos calls the session, the discovery and the naming functions below,
 from Objective-C. Its project file takes this package as a remote reference
 pinned to one exact version, so it compiles against a published tag rather than
 against this checkout, and a change here reaches it when somebody raises that
 version there.

 That is what makes `Scripts/check-callers.sh` worth running by hand rather than
 trusting: it runs that application's own gate against whatever its project file
 currently resolves, so it says the caller still builds, and it says nothing about
 declarations changed here since the version it is pinned to. Anything that
 changes a declaration below therefore needs that version raised in the caller as
 its own piece of work. It cannot run in CI either, because the runner has no copy
 of that application.
 */
...
/**
 ...
 @param port        Its port.
 @param senderName  What the receiver shows as the source, such as "Podlive".
                    NULL or empty, it shows "AirplayKit", because something has
                    to stand on the speaker and the library is what is sending.
