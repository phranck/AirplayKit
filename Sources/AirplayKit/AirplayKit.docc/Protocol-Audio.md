...
The three older open senders surveyed here use realtime audio: pyatv and airplay2-sender-cpp say so in their documentation, and owntone hardcodes the realtime payload type (reported confirmed for those implementations, [pyatv](https://github.com/postlund/pyatv), [airplay2-sender-cpp](https://github.com/akustikrausch/airplay2-sender-cpp) and [owntone, `src/outputs/airplay.c`](https://github.com/owntone/owntone-server/blob/master/src/outputs/airplay.c)). This library now implements buffered type 103 in `BufferedAudioStream` and uses it for its single-receiver sessions. That implementation alone does not provide multi-room synchronisation.
